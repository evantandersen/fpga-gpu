...
void transform_poly_list(scene_t *scene, render_target_t *target, polygon_list_t *out) {
	...
	clip_point_t *clip_vertices = malloc(sizeof(clip_point_t) * scene->poly_list.nvertices);
	uint8_t *clip_codes = malloc(scene->poly_list.nvertices);
	...
	triangle_t *kept_tris = malloc(sizeof(triangle_t) * scene->poly_list.ntris);
	size_t num_kept = 0;
	...
	screen_point_t *screen_vertices = malloc(sizeof(screen_point_t) * scene->poly_list.nvertices);
	for(size_t i = 0; i < scene->poly_list.nvertices; i++) {
...
struct triBin *bin_tris(int16_t tile_width, int16_t tile_height, polygon_list_t *data) {
	...
	//TODO dynamic bin size
	struct triBin *bins = malloc(sizeof(struct triBin) * nTiles);
	for(int i = 0; i < nTiles; i++) {
		bins[i].ntris = 0;
		bins[i].tris = malloc(sizeof(uint16_t) * data->ntris);
	}
...
void draw_triangles_barycentric_gpu(volatile gpu_t *GPU, render_target_t *target, polygon_list_t *data) {
    ...
    struct {
        ...
        float dzdy;
        float c;
    } *z_info = malloc(sizeof(float)*3*data->ntris);
...
//				triangle_t* curr = &data->triangles[];		
                ...
                //set the Z data
                volatile float *dzdx = (float*)&GPU->rasterizer[10];
                volatile float *dzdy = (float*)&GPU->rasterizer[11];
                volatile float *c = (float*)&GPU->rasterizer[12];
                *dzdx = z_info[tri].dzdx;
                *dzdy = z_info[tri].dzdy;
                *c = z_info[tri].c + j*32*z_info[tri].dzdx + i*32*z_info[tri].dzdy;
			...
			GPU->core[1] = ((uint32_t)(target->framebuffer)) + (i*32*target->stride) + (j*32*2);
...
void gpu_render_scene(volatile gpu_t *GPU, render_target_t *target, scene_t *scene) {
	...
	//render them
	draw_triangles_barycentric_gpu(GPU, target, &tris_to_render);
	
	free(tris_to_render.triangles);
	free(tris_to_render.vertices);
}
